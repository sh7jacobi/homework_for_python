...
class NTS(Optimizer):
    ...
    def __init__(self, params, function, subsolver=FGM, x_in=None, y_target=None, lr=1e-2, L=1, \
        ...
        defaults = dict(function=function, subsolver=subsolver, x_in=x_in, y_targer=y_target, lr=lr, L=L, epoch=epoch, 
    ...
    def step(self, closure=None):
        ...
        if self.defaults['flag_c']:
            self.defaults['F'], self.defaults['grad_F'] = self.func_and_grad()
        ...
        for i in range(self.defaults['epoch']):
            ...
            f_nes = self.get_f_nes(param_groups_k)
    ...
    def get_f_nes(self, param_groups_k):
        ...
        i = 0
        for F_i, grad_groups in zip(self.defaults['F'], self.defaults['grad_F']):
            ...
            #print('Jacobian * (x -y) : {} i_x : {}'.format(i, i_x))
            i += 1
